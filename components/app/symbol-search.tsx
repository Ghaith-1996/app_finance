    ...
    <div ref={containerRef} className={cn("relative", className)}>
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-black/8 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-950 outline-none transition focus:border-brand focus:ring-1 focus:ring-brand"
        />
        {loading && (
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-2xl border border-black/8 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.12)]">
          {results.map((r) => (
            <button
              ...
              className="flex w-full items-center justify-between px-4 py-3 text-left transition hover:bg-brand/6"
            >
              <div>
                <span className="text-sm font-semibold text-slate-950">{r.symbol}</span>
